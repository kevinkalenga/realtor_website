...
export default function Slider() {
  ...
  useEffect(() => {
    async function fetchListings() {
       ...
       const querySnape = await getDocs(q);
       let listings = [];
       querySnape.forEach((doc) => {
      ...
      <Swiper
         ...
         { listings.map(({data, id}) => (
             <SwiperSlide key={id} onClick={() =>navigate(`/category/${data.type}/${id}`)}>
                 ...
                 top-3 font-medium max-w-[90%] bg-[#457b9d] shadow-lg opacity-90 p-2
                 rounded-br-3xl'>
                  {data.name}</p>
                 <p className='text-[#f1faee] absolute left-1 
                 bottom-1 font-semibold max-w-[90%] bg-[#e63946] shadow-lg opacity-90 p-2
                 rounded-tr-3xl'>
